...
$db = mysqli_connect("127.0.0.1", "root", "1qaz@wsx", "travel_db");
...
if ($db) {
    $sql = "SELECT UID,title,content,city,dist,addr1 FROM blog ";
    $sql = $sql . "where title like '%" . $keyword . "%' ";
    $sql = $sql . "or content like '%" . $keyword . "%' ";
    $sql = $sql . "or city like '%" . $keyword . "%' ";
    $sql = $sql . "or dist like '%" . $keyword . "%' ";
    //$sql = $sql . "or addr1 like '%" . $keyword . "%' ";
    $sql = $sql . "order by UID,timestamp LIMIT 50";
    // echo $sql;
    // echo "<br>";
